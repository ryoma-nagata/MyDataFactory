...
3. RBACによりSQL ServerのロールをData Factoryに割り当て

##権限について
Data Warehouseの再開、停止は下記のような操作で実行されるため、最小の権限ロールを作成する場合は以下を実行できるロールを作成します。
...
###性能変更用Pipeline
名前を「[PauseAzureSQLDW」にして空のPipelineを作ります
![image.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/281819/1c8fa5a2-bb23-d1a9-b6ff-2244ce4dd86f.png)



##RBACによりSQL ServerのロールをData Factoryに割り当て
...
###まとめ
これを利用すればバッチ処理をData Factoryでのジョブ実行のフロー中に再開→性能アップ→処理→性能ダウン→休止が可能になります。
